...
namespace gl
{
	MeshAssimp::MeshAssimp(std::vector<VertexAssimp> vertices, std::vector<unsigned int> indices, std::vector<TextureAssimp> textures)
	{
	...
	void MeshAssimp::BindTexture(Shader& shader)
	{
	...
	void MeshAssimp::Draw(Shader& shader)
	{
		BindTexture(shader);
		// draw mesh
		glBindVertexArray(VAO);
	...
	void MeshAssimp::DrawInstancing(Shader& shader, MeshAssimp meshInstancing, unsigned int VBO_instances_, std::vector<glm::mat4> modelMatrix_)
	{
	...
	void MeshAssimp::Bind()
	{
		glBindVertexArray(VAO);
		IsError(__FILE__, __LINE__);
	}

	unsigned int MeshAssimp::GetVAO()
	{
		return VAO;
	}
}
